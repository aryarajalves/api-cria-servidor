...
class PostgresDeployDetails(BaseModel):
    ...
    postgres_password: str = Field(..., description="A senha a ser definida para o utilizador 'postgres' no banco de dados.")

# --- Instância do FastAPI ---
app = FastAPI(
    title="Postgres Deployer API",
    description="Uma API para implantar uma stack PostgreSQL usando a API do Portainer.",
    version="1.0.0"
)

# --- Template da Stack Postgres ---
POSTGRES_STACK_TEMPLATE = """
version: "3.7"
services:
  postgres:
    image: postgres:14
...
# --- Endpoint da API ---
@app.post("/deploy-postgres", status_code=status.HTTP_200_OK)
def deploy_postgres_stack(details: PostgresDeployDetails):
    """
    Autentica-se na API do Portainer e implanta ou atualiza a stack do PostgreSQL.
    """
    logs = []
    try:
        ...
        # Passo 4: Verificar se a stack "postgres" já existe
        logs.append("Passo 4: A verificar se a stack 'postgres' já existe...")
        ...
        postgres_stack = next((stack for stack in existing_stacks if stack.get("Name") == "postgres" and stack.get("EndpointId") == endpoint_id), None)

        # Prepara o conteúdo da stack, substituindo a senha
        stack_content = POSTGRES_STACK_TEMPLATE.replace("{{POSTGRES_PASSWORD}}", details.postgres_password)

        # Passo 5: Criar ou Atualizar a stack
        if postgres_stack:
            # Se existir, ATUALIZA a stack (PUT)
            stack_id = postgres_stack.get("Id")
            logs.append(f"Stack 'postgres' encontrada com ID {stack_id}. A atualizar...")
            ...
            logs.append("Stack 'postgres' não encontrada. A criar uma nova...")
        ...
        logs.append("Stack do PostgreSQL implantada com sucesso através do Portainer.")
