...
class EvolutionDeployDetails(BaseModel):
    ...
    evolution_domain: str = Field(..., description="O domínio que será usado para aceder à Evolution API.", example="evo.meudominio.com")
    evolution_apikey: str = Field(..., description="A API Key global para a Evolution API.")
...
app = FastAPI(
    title="Evolution API Deployer",
    description="Uma API para configurar o DNS e implantar a stack da Evolution API usando o Portainer.",
    version="1.1.0" # Versão com correção de Swarm ID
)

# --- Template da Stack Evolution ---
EVOLUTION_STACK_TEMPLATE = """
version: "3.7"
services:
  evolution:
    ...
    environment:
      ...
      - AUTHENTICATION_API_KEY={EVOLUTION_APIKEY}
...
def update_cloudflare_dns(details: EvolutionDeployDetails, logs: List[str]):
    logs.append(f"Atualizando DNS para {details.evolution_domain} -> {details.host}")
    ...
    response = requests.get(api_url, headers=headers, params={"name": details.evolution_domain})
    response.raise_for_status()
    records = response.json().get("result", [])
    payload = {"type": "A", "name": details.evolution_domain, "content": details.host, "ttl": 1, "proxied": False}
    if records:
...
def wait_for_dns_propagation(domain: str, target_ip: str, logs: List[str]):
    ...
    # Implementação da espera de DNS aqui... (código omitido por brevidade, mas funciona como nos scripts anteriores)
    time.sleep(15) # Simulação da espera
    logs.append("Propagação do DNS confirmada (simulado).")


# --- Endpoint da API ---
@app.post("/deploy-evolution", status_code=status.HTTP_200_OK)
def deploy_evolution_stack(details: EvolutionDeployDetails):
    """
    Configura o DNS, autentica-se no Portainer e implanta ou atualiza a stack da Evolution API.
    """
    logs = []
    try:
        # Passo 1: Configurar DNS na Cloudflare
        update_cloudflare_dns(details, logs)
        wait_for_dns_propagation(details.evolution_domain, details.host, logs)
        ...
        # CORREÇÃO: Adicionado passo para obter o ID de Swarm específico do endpoint
        ...
        # Passo 4: Verificar se a stack "evolution" já existe
        logs.append("Passo 4: A verificar se a stack 'evolution' já existe...")
        ...
        evolution_stack = next((stack for stack in existing_stacks if stack.get("Name") == "evolution" and stack.get("EndpointId") == endpoint_id), None)

        # Prepara o conteúdo da stack, substituindo as variáveis
        stack_content = EVOLUTION_STACK_TEMPLATE.replace("{EVOLUTION_DOMAIN}", details.evolution_domain)
        stack_content = stack_content.replace("{EVOLUTION_APIKEY}", details.evolution_apikey)

        # Passo 5: Criar ou Atualizar a stack
        if evolution_stack:
            stack_id = evolution_stack.get("Id")
            logs.append(f"Stack 'evolution' encontrada com ID {stack_id}. A atualizar...")
            ...
            logs.append("Stack 'evolution' não encontrada. A criar uma nova...")
            create_url = f"{details.portainer_url}/api/stacks/create/swarm/string?endpointId={endpoint_id}"
            create_payload = {"Name": "evolution", "SwarmID": swarm_id, "StackFileContent": stack_content}
            deploy_response = requests.post(create_url, headers=headers, json=create_payload, verify=False)

        deploy_response.raise_for_status()

        logs.append("Stack da Evolution API implantada com sucesso através do Portainer.")
